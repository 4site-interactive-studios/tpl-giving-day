(function () {
  ...
  function updateThermometer(count, goal, fillElm, numberElm, prefix) {
    ...
    function tick(ts) {
      ...
      numberElm.innerHTML =
        prefix + commafy(shown) + " of " + prefix + commafy(goal) + " goal";
    ...
    window.FUN.elements.addElementContent = function (payload) {
      try {
        if (
          ...
          if (
            raised !== null &&
            conf.isMatchAmountEnabled &&
            conf.matchingFactor > 1
          ) {
            raised *= conf.matchingFactor;
          }
          if (raised !== null) giveTotal = Math.round(raised);
          if (typeof conf.goalAmount === "number" && conf.goalAmount > 0)
            giveGoal = conf.goalAmount;
          renderGiveTherm();
        }
      } catch (e) {}
  ...
  if (typeof window.FundraiseUp === "function") {
    window.FundraiseUp.on("donationComplete", function (details) {
      if (
        !details ||
        !details.donation ||
        typeof details.donation.amount !== "number"
      )
        return;
  ...
  picked.forEach(function (d, i) {
    if (!rows[i]) return;
    rows[i].querySelector("td:nth-child(1)").innerHTML =
      d.name + ", <em>" + d.state + "</em>";
    rows[i].querySelector("td:nth-child(2)").textContent =
      "$" + commafy(d.amount);
  });
})();
